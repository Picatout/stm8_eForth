...
QKEY:
        CLRW Y 
        BTJF UART2_SR,#UART_SR_RXNE,INCH   ;check status
        LD    A,UART2_DR   ;get char in A
	SUBW	X,#2
        LD     (1,X),A
	CLR	(X)
        CPLW     Y
...
EMIT:
        LD     A,(1,X)
	ADDW	X,#2
OUTPUT: BTJF UART2_SR,#UART_SR_TXE,OUTPUT  ;loop until tdre
        LD    UART2_DR,A   ;send A
        RET
